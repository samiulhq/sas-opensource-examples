...
cas.table.fetch(session,table=c(caslib=currentCaslib,name="STENT_FAILURE"))
...
mtcars_cas=as.casTable(session,mtcars,casOut = c(name='mtcars',caslib='CASUSER',promote=TRUE))
...
mtcars_cas=cas.table.save(session,
                          ...
                          name='mtcars',#output file name
                          table=c(name='mtcars',caslib="CASUSER"), #source table information
                          replace=TRUE
                          )

cas.table.fileInfo(session,caslib=c(name="CASUSER"))

cas.upload.file(session,data='varimp.csv',casout=c(caslib='CASUSER',name='mytable'))
